...
namespace CineWorld.Services.MovieAPI.Controllers
{
  ...
  public class ServerAPIController : ControllerBase
  {
    ...
    public async Task<ActionResult<ResponseDto>> Get([FromQuery] ServerQueryParameters queryParameters)
    {
      ...
      int totalItems = await _unitOfWork.Server.CountAsync(query);
    ...
    public IActionResult GenerateSignedCookie()
    {
      ...
      string keyPairId = "K3JL4DK75BQ7V9";


      CloudFrontCookieHelper.SetSignedCookies(Response, resourceUrl, expiresOn, privateKeyPath, keyPairId);

      return Ok(new { message = "Signed cookies set successfully!" });
    }
  }
}
